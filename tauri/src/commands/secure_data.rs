...
#[tauri::command]
pub async fn load_secure_data(app_handle: tauri::AppHandle) -> Result<Option<VaultData>, String> {
    let path = app_handle.path().app_data_dir().unwrap().join("vault.bin");
...
#[tauri::command]
pub async fn clear_secure_data(app_handle: tauri::AppHandle) -> Result<(), String> {
    let path = app_handle.path().app_data_dir().unwrap().join("vault.bin");
    if path.exists() {
