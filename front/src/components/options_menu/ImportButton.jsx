  ...
  const handleImport = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        showToast(t('editor.options_menu.import_error'), "error");
        e.target.value = ""; // Reset file input to allow importing the same file again if needed
      }
    };
  ...
  return (
    <>
      ...
      <button 
        ...
        className="cursor-pointer flex items-center gap-2 px-3 py-2 text-sm text-left rounded-lg hover:bg-hover-primary-bg text-gray-700 dark:text-zinc-300 transition-colors w-full"
      >
